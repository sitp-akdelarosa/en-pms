...
$(function () {
    ...
    $('#tbl_jo_details_body').on('change', '.material_heat_no', function() {
        ...
        // console.log($(this).select2('data')); //.select2('data').element[0].attributes['data-name'].value);
    ...
    $('#tbl_jo_details_body').on('change', '.sched_qty', function(event) {
        ...
        if ($(this).val() == "") {
            ...
            if ($('#is_same').is(':checked')) {
                ...
                $('#btn_over_issuance_' + $(this).attr('data-count')).attr('data-sched_qty', total);
                ...
                $('#btn_over_issuance_' + $(this).attr('data-count')).attr('data-sched_qty', sched_qty);
    ...
    $('#btn_check_over_issuance').on('click', function() {
        ...
        var prod_code = $('input[name="prod_code[]"]').map(function () { return $(this).val(); }).get();
        var inv_id = $('input[name="inv_id[]"]').map(function () { return $(this).val(); }).get();
        var sched_qty = $('input[name="sched_qty[]"]').map(function () { return $(this).val(); }).get();
        var rmw_issued_qty = $('input[name="rmw_issued_qty[]"]').map(function () { return $(this).val(); }).get();
...
function makeJODetailsList(arr) {
    ...
    $('#tbl_jo_details').dataTable({
        ...
        columns: [
            ...
            { data: function(x) {
                return "<select id='material_used_" + x.count + "' data-count='" + x.count + "' data-pcode='" + x.prod_code +"' class='form-control form-control-sm material_used material_used_select' name='material_used[]'>"+
            ...
            { data: function(x) {
                return "<input type='number' step='1' id='sched_qty_" + x.count + "' data-pcode='" + x.prod_code + "' data-count='"+x.count+"' "+
            ...
            {
                data: function (x) {
                    return "<button type='button' id='btn_over_issuance_" + x.count + "' "+
                                "data-prod_code='" + x.prod_code + "' "+
                                "data-inv_id='" + x.inv_id + "' "+
                                "data-count='" + x.count + "' "+
                                "data-sched_qty='" + x.sched_qty + "' "+
                                "data-rmw_issued_qty='0' "+
                                "class='btn btn-sm bg-green btn_over_issuance' disabled>"+
                                "Check Issuance"+
                            "</button>";
                }, searchable: false, orderable: false
            },
