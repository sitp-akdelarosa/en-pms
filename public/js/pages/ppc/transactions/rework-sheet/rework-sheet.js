...
function validateInput(){
	var rows = $('#tbl_product')["0"].rows.length - 1;
 	if(rows > 0){
		...
		msg('Please prepare your rework sheet properly.', 'failed')
		return "invalid";
	}
	return  "NONE";
}
...
function SaveTravelSheet(){
	$('.loadingOverlay-modal').show();
	$.ajax({
		url: $('#frm_rework_sheet').attr('action'),
		type: 'POST',
		dataType: 'JSON',
		data: $('#frm_rework_sheet').serialize(),
	}).done(function(data, textStatus, xhr) {
		if (data.status == "success") {
			msg(data.msg,data.status);
			$('#rework_sheet_id').val(data.rework_sheet_id);
